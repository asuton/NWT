...
const DashboardEvents = ({ getEvents, auth, event: { events } }) => {
  ...
  let filteredUserEvents = events.filter(
    event =>
      event.user === auth.user._id && event.eventDate.slice(0, 10) === today
  );
